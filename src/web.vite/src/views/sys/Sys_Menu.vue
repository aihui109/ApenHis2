    ...
    <div class="menu-right">
      <el-scrollbar style="height: 100%">
        ...
        <div style="padding: 0px 30px 0 20px;margin-top: 15px;" class="form-content">
          <vol-form ref="form" :formRules="options" :formFields="fields">
          </vol-form>
          <div style="padding: 5px 21px;">
            <div style="padding-bottom: 10px;color: #737272; font-size: 14px;"><label style="width: 100px">权限按钮</label>
            </div>
            <div class="auth-group">

              <div class="ck">
                <el-checkbox-group v-model="actions">
                  <el-checkbox v-for="(item, index) in action" :key="index" :label="item.value"><span
                      style="top: -2px;position: relative;">{{ item.text + "(" + item.value + ")" }}</span></el-checkbox>
                </el-checkbox-group>
              </div>
            </div>
          </div>
          <div style="padding-left: 100px">
            <el-button @click="handleCheckAll" type="success" plain><i class="el-icon-check"></i>全 选</el-button>
            <el-button @click="actionModel = true" type="primary" plain><i class="el-icon-plus"></i>其他权限</el-button>
          </div>
          <div class="m-btn">
    ...
    const options = ref([
      ...
      [
        ...
        },
        {
          ...
          data: [
            ...
            { key: 1, value: "外部url链接" },
            { key: 3, value: "大屏设计(看板设计页面)" },
            { key: 4, value: "大屏列表(实际大屏页面)" }
          ],
        },
      ...
      [
        ...
        //   },
        {
          ...
          render: (h) => {
            return <div>
              <el-button type="primary" plain onClick={() => { model.value = true; }}>
                选择图标
              </el-button>
              <i style="font-size:25px;margin-left:10px;position:relative;top:4px" class={fields.value.icon}></i>
            </div>
            // return h("div", {}, [
            //   h(
