...
let dataType = [
  ...
  { key: 'selectBox', value: 'select弹出框选择' },
  { key: 'date', value: 'date' },
...
let data = {
  form: {
    fields: {
      ...
      editType: null,//编辑模式
      userPermissionDesc:
        '开启后当前用户只能操作自己(与下级角色)创建的数据,如:查询、删除、修改等操作'
    },
    addOptions: [
      [
        {
          ...
          type: 'number',
          placeholder:
            '放在【代码生成配置】列表的文件夹ID下,如果填入【0】就是一级目录'
        }
        ,
        {
          title: '项目类库',
          ...
          render: (h) => {
            return (
              <el-popover
                ...
                trigger="hover"
                content={
                  '必须先生成明细表配置(明细表只生成model即可，并且填写明细表的编辑行值>0)'
                }
              >
      ...
      [
        {
          ...
          field: 'editType',
          type: 'select',
          data: [{ key: 0, value: '弹出框编辑' }, { key: 1, value: '新页面编辑' }, { key: 2, value: '表格行内编辑' }],
          extra: {
            render: (h) => {
              return (
                <el-popover
