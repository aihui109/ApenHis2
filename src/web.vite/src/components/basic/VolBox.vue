<template>
  <div class="vol-dialog">
    ...
    <el-dialog v-model="vmodel" :destroy-on-close="destroyOnClose	" :close-on-click-modal="false" :close-on-press-escape="false" :width="width"
      ...
      <template #footer>
        <div class="dia-footer" v-if="footer">
          ...
          <el-button type="primary" v-if="!footer" size="mini" @click="handleClose()"><i
              class="el-icon-close"></i>{{ $ts("关闭") }}</el-button>
        </div>
      </template>
    </el-dialog>
  </div>
...
export default defineComponent({
  props: {
    ...
    destroyOnClose:{  //当关闭 Dialog 时，销毁其中的元素	
      type: Boolean,
      default: false
    }
