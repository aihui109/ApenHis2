<template>
  ...
  <el-form
    ...
    :rules="rules"
    :class="[
      labelPosition == 'left' || $global.labelPosition == 'left' ? 'form-normal' : 'form-lang'
    ]"
  >
        ...
        >
          ...
          <div v-else-if="isReadonlyImgFile(item, formFields)">
            ...
            <div
              ...
              :key="fileIndex"
            >
              <a @click="dowloadFile(formFields[item.field][fileIndex])">{{ file.name }}</a>
            </div>
          </div>
          ...
          <div v-else :class="{ 'form-item-extra': item.extra }">
            ...
            <vol-select-box
              ...
              :textInline="item.textInline"
              :paginationHide="item.paginationHide"
              :pagination="item.pagination"
              :loadBefore="item.loadBefore"
              :loadAfter="item.loadAfter"
              :url="item.url"
            ...
            <vol-select-table
              v-else-if="item.type == 'selectTable'"
              v-model="formFields[item.field]"
              :onSelect="item.onSelect"
              :textInline="item.textInline"
              :paginationHide="item.paginationHide"
            ...
            <div
              class="v-date-range"
              style="display: flex"
              v-else-if="['date', 'datetime', 'month'].indexOf(item.type) != -1 && item.range"
            >
              <el-date-picker
                :size="size"
                :disabled="item.readonly || item.disabled"
...
import upload from '@/components/basic/VolUpload.vue'
export default defineComponent({
  components: {
    FormExpand,
    'vol-upload': upload,
    'vol-wang-editor': defineAsyncComponent(() => import('@/components/editor/VolWangEditor.vue'))
  },
  props: {
    labelPosition: {
      type: String,
  ...
  setup(props, context) {
      ...
      props.formRules.forEach((row, xIndex) => {
        ...
        row.forEach((item, yIndex) => {
          ...
          if (item.type == 'number' || item.type == 'decimal') {
            if (props.formFields[item.field] === '') {
              props.formFields[item.field] = null
            }
            if (item.type == 'number') {
      ...
      await volform.value.validate((valid) => {
        if (!valid) {
          appContext.config.globalProperties.$message.error(
            appContext.config.globalProperties.$ts('数据验证未通过!')
          )
          result = false
    ...
    const currentGroup = ref(tabsGroup[0])
  ...
  created() {
    try {
      if (!this.$global.labelPosition) {
        this.$global.labelPosition = 'top'
      }
    } catch (error) {
  ...
  methods: {
    ...
    previewImg(url) {
      this.base.previewImg(url + this.access_token, this.http.ipAddress)
    },
    getSrc(path) {
      if (!path) return
    ...
    validatorPhone(ruleOption, value, callback) {
      ...
      if (!rule.phone.test((value || '').trim())) {
        return callback(new Error(this.$ts('请输入正确的手机号')))
      }
      callback()
    },
    validatorPwd(ruleOption, value, callback) {
      ...
      if ((value + '').trim().length < 6) {
        return callback(new Error(this.$ts('密码长度不能小于6位')))
      }
      callback()
    },
    ...
    getRule(item, formFields) {
        ...
        return {
          validator: (rule, val, callback) => {
            ...
            if (item.required && !this.isReadonly(item) && (!val || !val.length)) {
              return callback(new Error(this.$ts(item.type == 'img' ? '请上传照片' : '请上传文件')))
            }
            return callback()
          },
          required: item.required,
      ...
      if (
        ...
        // if (item.type=='decimal'&&!item.hasOwnProperty('precision')) {
        // //  item.precision=4;
        // }
          ...
          validator: (ruleObj, value, callback) => {
            ...
            if (ruleObj.type == 'number') {
              if (!rule.number.test(value)) {
                ruleObj.message = this.$ts([ruleObj.title, '只能是整数'], true)
                return callback(new Error(ruleObj.message))
              }
            } else {
              if (!rule.decimal.test(value)) {
                ruleObj.message = this.$ts([ruleObj.title, '只能是数字'], true)
                return callback(new Error(ruleObj.message))
              }
            }
            if (
      ...
      if (inputTypeArr.indexOf(item.type) != -1) {
        ...
        let _rule = {
          ...
          type: type,
          validator: (ruleObj, value, callback) => {
            if (!this.isReadonly(item) && (value === '' || value === undefined || value === null)) {
              return callback(new Error(ruleObj.message))
            }
            return callback()
          }
      ...
      if (item.type == 'radio') {
        return {
          required: item.required,
          message: this.$ts(['请选择', item.title], true),
          trigger: 'change',
          type: 'string'
        }
      }
        ...
        return {
          ...
          validator: (rule, val, callback) => {
            ...
            if (!val || (item.range && !val.length)) {
              return callback(new Error(this.$ts('请选择')))
            }
            return callback()
          }
        }
...
.vol-form-item {
  display: flex; // inline-block !important;
  margin-top: 4px;
}
// .vol-form-item:first-child{
//   margin-top: 0;
// }
...
.el-form-item ::v-deep(.el-input.is-disabled .el-input__wrapper),
.el-form-item ::v-deep(.el-textarea.is-disabled .el-textarea__inner) {
  background-color: #fafafa !important;
}
.el-form-item ::v-deep(.el-input.is-disabled .el-input__inner) {
...
// }
.form-lang ::v-deep(.el-form-item__label) {
  // justify-content: flex-end !important;
  margin-bottom: 0px !important;
}
.form-lang .vol-form-item {
  margin-bottom: -3px;
}
.form-normal {
...
.form-normal ::v-deep(.el-form-item__label) {
  justify-content: flex-end !important;
  padding-right: 4px;
}
.form-normal ::v-deep(.el-form-item) {
  padding: 0 !important;
}
...
//   margin-bottom: 10px;
// }
.form-tabs ::v-deep(.el-tabs__content) {
  padding: 0;
}
.form-tabs ::v-deep(.el-tabs__nav-wrap) {
  margin-bottom: 0;
}
...
.form-tabs ::v-deep(.el-tabs__nav-wrap:after),
.form-tabs ::v-deep(.el-tabs__active-bar) {
  height: 1px;
}
</style>
