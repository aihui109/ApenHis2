            ...
            <div @click.stop class="e-item">
              <div>
                ...
                <el-switch
                  ...
                  :active-text="$ts(column.activeText || '是')"
                  :inactive-text="$ts(column.inactiveText || '否')"
                  @change="
                    (val) => {
                ...
                <vol-select-table
                  ...
                  "
                  @onKeyPress="(value,$event)=>{column.onKeyPress&&column.onKeyPress(value,$event,scope.row)}"
                  :url="column.url"
...
export default defineComponent({
  ...
  created() {
    ...
    let keys = [];
    let columnBind = [];
    this.summaryData.push(this.$ts("合计"));
  ...
  methods: {
    ...
    rowBeginEdit(row, column) {
      ...
      let col= this.columns.find(x=>{return x.field==(column.field||column.property)});
      if (col&&col.edit) {
    ...
    delRow() {
      ...
      if (rows.length == 0) return this.$Message.error(this.$ts("请选择要删除的行!"));
    ...
    viewImg(row, column, url, $event,index) {
      $event && $event.stopPropagation();
        const imgs=  this.getFilePath(row[column.field], column).map(x=>{return x.path+this.access_token});
    ...
    updateSummary(field){//2024.03.05增加手动刷新合计方法
        let column=this.columns.find(x=>{return x.field==field})
        if (!column) {
          return;
        }
        this.getInputSummaries(null, null, null,column);
    ...
    getInputSummaries(scope, val, event, column) {
      ...
      (this.url ? this.rowData : this.tableData).forEach((x, index) => {
       ...
       if (column.summaryFormatter) {
            sum = column.summaryFormatter(sum,column,(this.url ? this.rowData : this.tableData),this.summaryData)
        }else{
          sum = (sum * 1.0)
          if (sum) {
    ...
    initDrag(){
      ...
      oDragIcon.onmousedown = (ev)=>{
       ...
       let   disH = refTable.offsetHeight; // 获取拖拽前div的高
