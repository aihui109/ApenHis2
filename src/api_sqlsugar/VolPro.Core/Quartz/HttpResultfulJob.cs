    ...
    public class HttpResultfulJob : IJob
    {
        ...
        public async Task Execute(IJobExecutionContext context)
        {
            ...
            try
            {
                ...
                var _taskOptions = DbManger.SysDbContext.Set<Sys_QuartzOptions>()
                      .Where(x => x.Id == taskOptions.Id).FirstOrDefault();

                if (_taskOptions != null)
                {
                    _taskOptions.LastRunTime = DateTime.Now;
                    DbManger.SysDbContext.Update(_taskOptions, new string[] { "LastRunTime" });

                    DbManger.SysDbContext.SaveChanges();
                }
