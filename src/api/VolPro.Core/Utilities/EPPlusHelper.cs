...
namespace VolPro.Core.Utilities
{
    public class EPPlusHelper
    {
        ...
        public static WebResponseContent ReadToDataTable<T>(string path,
            ...
            Expression<Func<T, Dictionary<string, string>>> headerMap = null)
        {
                    ...
                    for (int j = sheet.Dimension.Start.Column, k = sheet.Dimension.End.Column; j <= k; j++)
                    {
                        ...
                        if (!string.IsNullOrEmpty(options.DropNo) && !string.IsNullOrEmpty(value))
                        {
                            ...
                            if (key == null)//&& options.Requierd
                            {
                                ...
                                string values = options.KeyValues.Count < 100 ? (string.Join(',', options.KeyValues.Select(s => s.Value))) : options.ColumnCNName;
        ...
        private static Dictionary<string, string> GetHeaderMap<T>(Expression<Func<T, Dictionary<string, string>>> headerMap = null)
        {
            if (headerMap == null)
            {
                return null;
