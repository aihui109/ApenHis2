...
DISTINCT
	...
	SELECT CASE WHEN t.ColumnType IN ('DECIMAL','smallmoney','money') THEN 
        ...
        private string GetOracleStructure(string tableName)
        {
			...
			CASE WHEN  c.DATA_TYPE IN('smallint', 'INT') OR  (c.DATA_TYPE='NUMBER' AND c.DATA_SCALE=0) THEN 'int'  
        ...
        public string CreateEntityModel(Sys_TableInfo sysTableInfo)
        {
            ...
            List<TableColumnInfo> tableColumnInfoList = DBServerProvider.GetSqlDapper(connection).QueryList<TableColumnInfo>(sql, new { tableName });
        ...
        private string GetCurrentSql(string tableName, string connection)
        {
            ...
            else if (name == DbCurrentType.PgSql.ToString().ToLower())
            {
        ...
        private int InitTable(int parentId, string tableName, string columnCNName, string nameSpace, string foldername, int tableId, bool isTreeLoad, string dbServer)
        {
            ...
            string connection = GetConnectionKey(tableInfo);
            List<Sys_TableColumn> columns = DBServerProvider.GetSqlDapper(connection)
