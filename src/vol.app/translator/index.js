...
let $tsArr = function(arr, toLower) {
	if (Vue.prototype.isCN) return arr.join('');
	let ts_string = '';
...
function $changeSource(langType, isInit, source) {
	...
	Vue.prototype.isCN = currentLang == 'zh-CN';
	if (Vue.prototype.isCN) {
		source = {};
...
import tabBarChange from '@/translator/tabBarChangeLanguage.js'

export default {
	init(useLang) {
		Vue.prototype.$tsTabBar = tabBarChange;
		Vue.prototype.useLang = useLang;

		Vue.prototype.$ts = $ts;
		Vue.prototype.$tst = $tst;
		Vue.prototype.$tsArr = $tsArr;
		if (!useLang) {
			return;
		}
		$loadSource();
		Vue.prototype.$changeSource = $changeSource;
	}
}
