...
if (process.env.NODE_ENV == 'development') {
  axios.defaults.baseURL = 'http://localhost:9100/api/dataview/'
} else if (process.env.NODE_ENV == 'debug') {
  axios.defaults.baseURL = 'http://localhost:9100/api/dataview/'
} else if (process.env.NODE_ENV == 'production') {
  axios.defaults.baseURL = 'http://42.194.133.241:9100/api/dataview/'
}
//axios.defaults.baseURL = 'http://proapi.volcore.xyz/';
// if (!axios.defaults.baseURL.endsWith('/')) {
//   axios.defaults.baseURL += '/';
// }

sessionStorage.setItem('serviceUrl',axios.defaults.baseURL?.split('api/')[0]+'')
//window.$vue.serviceUrl=axios.defaults.baseURL?.split('api/')[0];
// window['serviceUrl']=axios.defaults.baseURL?.split('api/')[0];
...
export const post = <T = any>(url: string, data?: any, headersType?: string) => {
  //提交的参数名叫object,后台就不处理了，否则接收不了object参数名
  if (url.endsWith('upload') && data && data.get && data.get('object')) {
    const formData = data.get('object')
    data.append('fileInput', formData);
    data.set('object','')
    delete data.object
  }
  return axiosInstance<T>({
